...
struct MemoryGame<CardContent: Equatable> {
    ...
    mutating func choose(_ card: Card) {
        if let choosingIndex = cards.firstIndex(where: { $0.id == card.id }) {
            if !cards[choosingIndex].isFaceUp && !cards[choosingIndex].isMatched {
                if let potentialIndex = indexOfTheOnlyFaceUpCard {
                    if cards[choosingIndex].content == cards[potentialIndex].content {
                        cards[choosingIndex].isMatched = true
                        cards[potentialIndex].isMatched = true
                        score += 2 + cards[choosingIndex].bonus + cards[potentialIndex].bonus
                    } else {
                        if cards[choosingIndex].hasBeenSeen {
                            score -= 1
                        }
                    ...
                    indexOfTheOnlyFaceUpCard = choosingIndex
                }
                cards[choosingIndex].isFaceUp = true
            }
        }
    }
